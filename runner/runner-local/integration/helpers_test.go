...
func runCmd(t *testing.T, dir, name string, args ...string) string {
	t.Helper()
	out, err := runCmdNoFail(dir, name, args...)
	if err != nil {
...
func runCmdWithEnv(t *testing.T, dir string, env map[string]string, name string, args ...string) string {
	t.Helper()
...
func runCmdNoFail(dir, name string, args ...string) (string, error) {
	cmd := exec.Command(name, args...)
	cmd.Dir = dir
	data, err := cmd.CombinedOutput()
